...
function getAllFiles(basePath, dirPaths, arrayOfFiles, excludeDirs) {
  ...
  for (const dirPath of dirPaths) {
    ...
    files.forEach((file) => {
      ...
      if (fs.statSync(filePath).isDirectory()) {
        ...
        arrayOfFiles.push('/'+ path.relative(basePath, filePath).replace(/\\/g, '/'));
      }
    });
  }
  return arrayOfFiles;
...
const DEFAULT_INPUT_DIRS = [ "data", "js", "img" ];
...
function showHelp() {
  log.log(`
Usage: node cache-tools.js <command> [--input-dirs <comma-separated-dir-names>] [--output-file <file-name>] [--exclude-dirs <comma-separated-dir-names>] [--log-level <level>]

Defaults:
  input-dirs  : '${DEFAULT_INPUT_DIRS.join("', '")}'
  output-file : '${DEFAULT_OUTPUT_FILE}'
  exclude-dirs: '${DEFAULT_EXCLUDE_DIRS.join("', '")}'
...
Commands:
  ...
  generate
    List all files in the input directories and save the list as a JSON array in the output file    
  `);
}
...
async function main() {
  ...
  const inputDirsIndex = args.indexOf('--input-dirs');
  const inputDirs = inputDirsIndex !== -1 ? args[inputDirsIndex + 1].split(',').map(dir => dir.trim()) : DEFAULT_INPUT_DIRS;
  ...
  const excludeDirs = excludeDirsIndex !== -1 ? args[excludeDirsIndex + 1].split(',').map(dir => dir.trim()) : DEFAULT_EXCLUDE_DIRS;
  ...
  switch (command) {
    ...
    case 'generate':
      log.info(`Collecting all files in: '${inputDirs.join("', '")}'...`);	
      const fileList = getAllFiles('.', inputDirs, null, excludeDirs);
